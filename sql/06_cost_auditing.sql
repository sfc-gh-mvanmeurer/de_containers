/*
================================================================================
FGCU Canvas Data Engineering - Cost Auditing & Monitoring
================================================================================
...
VALUES 
    ('WAREHOUSE', 'FGCU_INGESTION_WH', 'DATA_PLATFORM', 'IT', 'CANVAS_ETL'),
    ('WAREHOUSE', 'FGCU_TRANSFORM_WH', 'DATA_PLATFORM', 'IT', 'CANVAS_ETL'),
    ('WAREHOUSE', 'FGCU_TASK_WH', 'DATA_PLATFORM', 'IT', 'CANVAS_ETL'),
    ('CONTAINER', 'CANVAS_ETL_SERVICE', 'DATA_PLATFORM', 'IT', 'CANVAS_ETL'),
    ('COMPUTE_POOL', 'FGCU_CANVAS_POOL', 'DATA_PLATFORM', 'IT', 'CANVAS_ETL');
...
WHERE wmh.start_time >= DATEADD('day', -30, CURRENT_DATE())
    AND wmh.warehouse_name LIKE 'FGCU_%'
GROUP BY 1, 2, 3, 4, 5
...
WHERE start_time >= DATEADD('day', -30, CURRENT_DATE())
    AND compute_pool_name LIKE 'FGCU_%'
GROUP BY 1, 2
ORDER BY 1 DESC;
...
WHERE spcs.start_time >= DATEADD('day', -30, CURRENT_DATE())
    AND spcs.compute_pool_name LIKE 'FGCU_%'
GROUP BY 1, 2, 3, 4, 5
ORDER BY 1 DESC;
...
WHERE query_start_time >= DATEADD('day', -30, CURRENT_DATE())
    AND database_name = 'FGCU_CANVAS_DEMO'
GROUP BY 1, 2, 3, 4, 5
ORDER BY 1 DESC, total_credits DESC;
...
WHERE table_catalog = 'FGCU_CANVAS_DEMO'
    AND deleted IS NULL
