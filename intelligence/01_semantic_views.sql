...
CREATE OR REPLACE SEMANTIC VIEW CANVAS_STUDENT_ANALYTICS
  ...
  DIMENSIONS (
    ...
    academic_standing AS 
      CASE 
        WHEN students.gpa >= 3.5 THEN 'Deans List'
  ...
  METRICS (
    ...
    deans_list_students AS COUNT_IF(students.gpa >= 3.5)
      DESCRIPTION 'Number of students on Deans List (GPA >= 3.5)',
    graduation_rate AS 
...
CREATE OR REPLACE SEMANTIC VIEW CANVAS_PERFORMANCE_DASHBOARD
  ...
  METRICS (
    ...
    students_at_risk AS 
      ...
      DESCRIPTION 'Number of students scoring below 60 percent',
    high_performers AS 
      COUNT_IF(performance.average_score >= 90)
      DESCRIPTION 'Number of students scoring 90 percent or above'
  )
